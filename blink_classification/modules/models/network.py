...
class FaceNet(torch.nn.Module):
    ...
    def __init__(
        ...
        if model_type == 'simple':
            self.backbone = self.__create_simple_backbone_model(in_channels=in_channels)
        ...
        self.classifier = torch.nn.Linear(
            backbone_out_channels * 5 * 5, num_of_output_nodes
        )

    @staticmethod
    def __create_simple_backbone_model(in_channels: int = 3) -> torch.nn.Module:
        model = torch.nn.Sequential(
            torch.nn.Conv2d(
                in_channels=in_channels, out_channels=6, kernel_size=(3, 3)
            ),
            torch.nn.MaxPool2d(kernel_size=(2, 2), stride=2),
            torch.nn.Conv2d(in_channels=6, out_channels=16, kernel_size=(3, 3)),
            torch.nn.MaxPool2d(kernel_size=(2, 2), stride=2),
            torch.nn.Conv2d(
